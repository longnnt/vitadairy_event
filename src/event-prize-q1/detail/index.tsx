...
export default function EventPrizeQ1Detail() {
  ...
  const { data: crmTransaction } = useGetCrmTransactionById(
    prizeDataDetail?.crmTransactionTypeId || 0
  );
        ...
        <Container maxWidth={themeStretch ? 'sm' : 'xl'}>
          <HeaderBreadcrumbs
            ...
            links={[
              ...
              },
              {
                ...
                href: replacePathParams(PATH_DASHBOARD.eventPrizeQ1.list, {
                  eventId: eventId || '',
                }),
              },
              {
          ...
          <Paper elevation={3} sx={{ p: 3 }}>
            ...
            <Stack direction={'row'} spacing={3} sx={{ pt: 3 }}>
              <TextField
                ...
                sx={{ width: '50%' }}
                value={
                  dayjs(prizeDataDetail?.startDate).isValid()
                    ? dayjs(prizeDataDetail?.endDate).format(FORMAT_DATE_FILTER)
                    : ''
                }
                disabled
                InputLabelProps={{ shrink: true }}
              ...
              <TextField
                ...
                sx={{ width: '50%' }}
                value={
                  dayjs(prizeDataDetail?.endDate).isValid()
                    ? dayjs(prizeDataDetail?.endDate).format(FORMAT_DATE_FILTER)
                    : ''
                }
                disabled
            ...
            <Stack spacing={5} direction="row" sx={{ pt: 3 }}>
              <Box sx={{ width: '50%' }}>
                ...
                >
                  ...
                  <Switch
                    checked={prizeDataDetail?.status}
                    onClick={(e) => e.preventDefault()}
                  />
                </Stack>
              </Box>
            ...
            <>
              ...
              <Box sx={{ mt: 3, mb: 3 }}>
                <Card sx={{ width: '20%', px: 4, py: 2 }}>
                  ...
                  >
                    <Typography variant="body1">
                      Tổng số giải <br />ở tất cả tỉnh thành
                    </Typography>
              ...
              <Paper elevation={3}>
                <TableContainer>
                  <Table size="medium">
                    ...
                    <TableBody>
                      {prizeDataDetail &&
                        prizeDataDetail?.eventDetailProvinces?.map(
                          (item: IProvinceDetail) => (
                            <TableRow key={item.provinceId}>
                              <TableCell align="center">{item.provinceName}</TableCell>
                              <TableCell align="center">{item.quantity}</TableCell>
                              <TableCell align="center">
                                {dayjs(item?.startDate).isValid()
                                  ? dayjs(item?.startDate).format(FORMAT_DATE_FILTER)
                                  : ''}
                              </TableCell>
                              <TableCell align="center">
                                {dayjs(item?.endDate).isValid()
                                  ? dayjs(item?.endDate).format(FORMAT_DATE_FILTER)
                                  : ''}
                              </TableCell>
                              <TableCell align="center">{item.wonAmount}</TableCell>
                            </TableRow>
                          )
                        )}
                      <TableNoData
                        isNotFound={
                          !prizeDataDetail ||
                          prizeDataDetail?.eventDetailProvinces?.length === 0
                        }
                      />
                    </TableBody>
                  </Table>
          ...
          >
            <Button
              ...
              onClick={() => {
                navigate(
                  replacePathParams(PATH_DASHBOARD.eventPrizeQ1.list, {
                    eventId: prizeDataDetail?.eventId,
                  })
                );
              }}
            >
